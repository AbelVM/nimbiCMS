...
export async function generateSitemapJson(opts = {}) {
  ...
  let idx =
    Array.isArray(searchIndex) && searchIndex.length
      ? searchIndex
      : Array.isArray(providedIndex)
        ? providedIndex
        : [];
  if (
    Array.isArray(providedIndex) &&
    providedIndex.length &&
    Array.isArray(searchIndex) &&
    searchIndex.length
  ) {
    const bySlug = new Map();
    try {
      for (const it of providedIndex) {
        try {
          if (it?.slug) bySlug.set(String(it.slug), it);
        } catch {}
      }
  ...
  try {
    if (typeof notFoundPage === "string" && notFoundPage.trim())
      excludedPaths.add(normalizePath(String(notFoundPage)));
  } catch {}
  try {
    if (typeof navigationPage === "string" && navigationPage.trim())
      excludedPaths.add(normalizePath(String(navigationPage)));
  } catch {}
    ...
    if (typeof notFoundPage === "string" && notFoundPage.trim()) {
      ...
      try {
        if (typeof mdToSlug?.has === "function" && mdToSlug.has(nfPath)) {
          try {
            excludedSlugs.add(mdToSlug.get(nfPath));
          } catch {}
        } else {
          try {
            const nfRes = await fetchMarkdown(
              nfPath,
              opts?.contentBase ? opts.contentBase : undefined,
            );
            if (nfRes?.raw) {
              try {
                let h = null;
  ...
  const _isKnownPath = (p) => {
    try {
      ...
      try {
        if (
          typeof allMarkdownPathsSet?.has === "function" &&
          allMarkdownPathsSet.has(np)
        )
          return true;
      } catch {}
      try {
        if (typeof mdToSlug?.has === "function" && mdToSlug.has(np))
          return true;
      } catch {}
      try {
        if (pathMap?.has(np)) return true;
      } catch {}
      try {
        ...
        if (typeof mdToSlug?.keys === "function" && mdToSlug.size) {
          for (const k of mdToSlug.keys()) {
            try {
              if (normalizePath(String(k)) === np) return true;
            } catch {}
          }
        } else {
          for (const v of slugToMd.values()) {
            try {
              ...
              if (typeof v === "string") {
                ...
                if (v.default && normalizePath(String(v.default)) === np)
                  return true;
                const langs = v.langs || {};
                for (const lk of Object.keys(langs || {})) {
                  try {
                    if (langs[lk] && normalizePath(String(langs[lk])) === np)
                      return true;
                  } catch {}
                }
              }
  ...
  if (Array.isArray(idx) && idx.length) {
    ...
    for (const it of idx) {
      ...
      try {
        ...
        const entryTitle = it.title
          ? String(it.title)
          : it.parentTitle
            ? String(it.parentTitle)
            : undefined;
        titleMap.set(slugKey, {
    ...
    try {
      ...
      for (const [slug, mdVal] of slugToMd.entries()) {
        ...
        try {
          ...
          if (typeof mdVal === "string")
            mappedPath = normalizePath(String(mdVal));
          else if (mdVal && typeof mdVal === "object")
            mappedPath = normalizePath(String(mdVal.default ?? ""));
          ...
          if (typeof slug === "string") {
            const looksLikePath =
              slug.indexOf("/") !== -1 || /\.(md|html?)$/i.test(slug);
            const titleLooksLikePath =
              ent.title &&
              typeof ent.title === "string" &&
              (ent.title.indexOf("/") !== -1 ||
                /\.(md|html?)$/i.test(ent.title));
      ...
      try {
        if (homePage && typeof homePage === "string") {
          ...
          let hpSlug = null;
          try {
            if (typeof mdToSlug?.has === "function" && mdToSlug.has(hp))
              hpSlug = mdToSlug.get(hp);
          } catch {}
          if (!hpSlug) hpSlug = hp;
          const hpBase = String(hpSlug).split("::")[0];
          if (
            !seenSlugs.has(hpSlug) &&
            !excludedPaths.has(hp) &&
            !excludedSlugs.has(hpBase)
          ) {
            const ent = {
              loc: baseNoQs + "?page=" + encodeURIComponent(hpSlug),
              slug: hpSlug,
            };
  ...
  try {
    ...
    for (const it of entries) {
      try {
        if (it?.sourcePath) sourcePaths.add(String(it.sourcePath));
      } catch {}
    }
    ...
    for (const sp of sourcePaths) {
      ...
      try {
        ...
        fetched += 1;
        const mdRes = await fetchMarkdown(
          sp,
          opts?.contentBase ? opts.contentBase : undefined,
        );
        ...
        if (mdRes && typeof mdRes.status === "number" && mdRes.status === 404)
          continue;
        const raw = mdRes.raw;
              ...
              try {
                ...
                if (p) {
                  ...
                  if (!existingSlugs.has(slug) && !seenAdd.has(slug)) {
                    seenAdd.add(slug);
                    entries.push({
                      loc: baseNoQs + "?page=" + encodeURIComponent(slug),
                      slug,
                    });
                  }
                  continue;
            ...
            try {
              ...
              if (mdToSlug?.has?.(norm)) {
                ...
                if (
                  ...
                  !existingSlugs.has(s) &&
                  !seenAdd.has(s) &&
                  !excludedSlugs.has(sBase) &&
                  !excludedPaths.has(norm)
                ) {
                  seenAdd.add(s);
                  entries.push({
                    loc: baseNoQs + "?page=" + encodeURIComponent(s),
                    slug: s,
                    sourcePath: norm,
                  });
                }
              ...
              try {
                ...
                const target = await fetchMarkdown(
                  norm,
                  opts?.contentBase ? opts.contentBase : undefined,
                );
                ...
                if (target && target.raw) {
                  ...
                  if (
                    s2 &&
                    !existingSlugs.has(s2) &&
                    !seenAdd.has(s2) &&
                    !excludedSlugs.has(s2Base)
                  ) {
                    seenAdd.add(s2);
                    entries.push({
                      loc: baseNoQs + "?page=" + encodeURIComponent(s2),
                      slug: s2,
                      sourcePath: norm,
    ...
    for (const base of basesNeeded) {
      try {
        ...
        if (titleMap.has(base)) {
          ...
          ent = {
            loc: baseNoQs + "?page=" + encodeURIComponent(base),
            slug: base,
          };
          if (t?.title) {
            ent.title = t.title;
  ...
  try {
    try {
      debugLog(
        () =>
          "[runtimeSitemap] generateSitemapJson finalEntries.titleSource: " +
          JSON.stringify(
            final.map((e) => ({
              slug: e.slug,
              title: e.title,
              titleSource: e._titleSource || null,
            })),
            null,
            2,
          ),
  ...
  try {
    ...
    const workers = Array.from({
      length: Math.min(MAX_FETCH_CONCURRENCY, total),
    }).map(async () => {
      while (true) {
        const i = _nextIndex++;
        if (i >= total) break;
        ...
        try {
          ...
          try {
            if (slugToMd?.has?.(e.slug)) {
              ...
              if (typeof mv === "string")
                candidatePath = normalizePath(String(mv));
              else if (mv && typeof mv === "object")
                candidatePath = normalizePath(String(mv.default ?? ""));
            }
          ...
          try {
            ...
            if (md && typeof md.status === "number" && md.status === 404)
              continue;
            if (md && md.raw) {
            ...
            debugLog(
              "[runtimeSitemap] fetch title failed for",
              candidatePath,
              err,
            );
          }
        } catch (err) {
          debugLog("[runtimeSitemap] worker loop failure", err);
        }
      }
    });
    await Promise.all(workers);
  } catch (err) {
    debugLog("[runtimeSitemap] title enrichment failed", err);
  }
...
export function generateRssXml(json) {
  ...
  for (const e of entries) {
    try {
      ...
      if (e.excerpt)
        s += `<description>${_escapeXml(String(e.excerpt))}</description>\n`;
...
export function generateAtomXml(json) {
  const entries = Array.isArray(json?.entries)
    ...
    : Array.isArray(json)
      ...
      : [];
  const base = _getBase().split("?")[0];
  const updated = json?.generatedAt
    ? new Date(json.generatedAt).toISOString()
  ...
  for (const e of entries) {
    try {
      const loc = String(e.loc ?? "");
      const entryUpdated = e?.lastmod
        ? new Date(e.lastmod).toISOString()
        : updated;
      s += "<entry>\n";
      s += `<title>${_escapeXml(String(e.title || e.slug || (e.loc ?? "")))}</title>\n`;
      if (e.excerpt)
        s += `<summary>${_escapeXml(String(e.excerpt))}</summary>\n`;
  ...
  try {
    ...
    try {
      try {
        ...
        try {
          pre.textContent = _escapeXml(xml);
        } catch (err) {
          try {
            pre.textContent = String(xml);
          } catch (_) {}
        }
        if (document && document.body) {
          try {
            if (typeof document.body.replaceChildren === "function")
            ...
            try {
              document.body.innerHTML = "<pre>" + _escapeXml(xml) + "</pre>";
            } catch (_) {}
          }
        }
      } catch (err) {}
...
function _generateHtmlFromJson(finalJson) {
  try {
    const entries = Array.isArray(finalJson?.entries)
      ? finalJson.entries
      : Array.isArray(finalJson)
        ? finalJson
        : [];
    let html =
...
function _scheduleSitemapWrite(finalJson, mimeType = "application/xml") {
  try {
    ...
    try {
      ...
      if (
        !pending ||
        (typeof pending.len === "number" && pending.len < newLen)
      ) {
        window.__nimbiSitemapPendingWrite = {
          finalJson,
          mimeType,
          len: newLen,
        };
      ...
      window.__nimbiSitemapWriteTimer = setTimeout(() => {
        try {
          ...
          if (!p) return;
          let out = null;
          if (p.mimeType === "application/rss+xml")
            out = generateRssXml(p.finalJson);
          else if (p.mimeType === "application/atom+xml")
  ...
  try {
    ...
    try {
      if (typeof whenSearchIndexReady === "function") {
        try {
          const live = await whenSearchIndexReady({
            timeoutMs: waitMs,
            contentBase: opts?.contentBase,
            indexDepth: opts?.indexDepth,
            noIndexing: opts?.noIndexing,
            startBuild: true,
          });
      ...
      if (Array.isArray(opts.index) && opts.index.length) {
        try {
          ...
          for (const it of opts.index) {
            try {
              ...
              if (!map.has(base)) map.set(base, it);
                ...
                if (
                  prev &&
                  String(prev.slug ?? "").indexOf("::") !== -1 &&
                  String(it.slug ?? "").indexOf("::") === -1
                ) {
                  map.set(base, it);
                }
              }
          ...
          try {
            debugLog(
              () =>
                "[runtimeSitemap] providedIndex.dedupedByBase: " +
                JSON.stringify(Array.from(map.values()), null, 2),
            );
          } catch (e) {
            debugLog(
              () =>
                "[runtimeSitemap] providedIndex.dedupedByBase (count): " +
                String(map.size),
            );
          }
        } catch (e) {
          debugWarn("[runtimeSitemap] logging provided index failed", e);
        }
      }
    } catch (e) {}
    ...
    if (
      ...
      try {
        const waitMs2 =
          typeof opts.waitForIndexMs === "number"
            ? opts.waitForIndexMs
            : Infinity;
        let maybe = null;
        try {
          if (typeof whenSearchIndexReady === "function") {
            maybe = await whenSearchIndexReady({
              timeoutMs: waitMs2,
              contentBase: opts?.contentBase,
              indexDepth: opts?.indexDepth,
              noIndexing: opts?.noIndexing,
        ...
        if (Array.isArray(maybe) && maybe.length) {
          ...
          const indexDepth =
            typeof opts.indexDepth === "number" ? opts.indexDepth : 3;
        ...
        idx =
          Array.isArray(searchIndex) && searchIndex.length ? searchIndex : [];
      }
    }
    ...
    try {
      ...
      try {
        debugLog(
          () =>
            "[runtimeSitemap] usedIndex.full.length (before rebuild): " +
            String(len),
        );
      } catch (e) {}
      try {
        debugLog(
          () =>
            "[runtimeSitemap] usedIndex.full (before rebuild): " +
    ...
    try {
      ...
      const rebuildDepth =
        typeof opts.indexDepth === "number" ? opts.indexDepth : 3;
      const rebuildNoIndexing = Array.isArray(opts.noIndexing)
        ? opts.noIndexing
        : undefined;
      let rebuilt = null;
      try {
        const workerFn =
          ...
          typeof globalThis.buildSearchIndexWorker === "function"
            ? globalThis.buildSearchIndexWorker
            : undefined;
        if (typeof workerFn === "function") {
          try {
            rebuilt = await workerFn(
              opts?.contentBase,
              rebuildDepth,
              rebuildNoIndexing,
            );
      ...
      if (
        ...
        try {
          rebuilt = await buildSearchIndex(
            opts?.contentBase,
            rebuildDepth,
            rebuildNoIndexing,
      ...
      try {
        debugWarn("[runtimeSitemap] rebuild index call failed", e);
      } catch (_) {}
    }
    ...
    try {
      ...
      try {
        debugLog(
          () =>
            "[runtimeSitemap] usedIndex.full.length (after rebuild): " +
            String(len2),
        );
      } catch (e) {}
      try {
        debugLog(
          () =>
            "[runtimeSitemap] usedIndex.full (after rebuild): " +
            JSON.stringify(idx, null, 2),
        );
      } catch (e) {
        /* ignore stringify errors */
    ...
    try {
      ...
      try {
        debugLog(
          () =>
            "[runtimeSitemap] finalEntries.dedupedByBase: " +
            JSON.stringify(deduped, null, 2),
        );
      } catch (e) {
        debugLog(
          () =>
            "[runtimeSitemap] finalEntries.dedupedByBase (count): " +
    ...
    const finalJson = Object.assign({}, json || {}, {
      entries: Array.isArray(deduped)
        ? deduped
 ...
 * @param {HTMLElement} mount - Container where the control should be appended.
 * @param {{filename?: string}} [opts] - Optional UI options.
 * @returns {HTMLElement|null} The created button element, or null when not attached.
 */
export function attachSitemapDownloadUI(mount, opts = {}) {
  try {
    if (!mount || typeof document === "undefined") return null;
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "button is-small is-light";
    btn.textContent = "Download sitemap";
    btn.setAttribute("aria-label", "Download sitemap JSON");
    btn.addEventListener("click", async () => {
      try {
        ...
        debugWarn("[runtimeSitemap] attachSitemapDownloadUI click failed", e);
      }
    });
    mount.appendChild(btn);
    return btn;
  } catch (e) {
    debugWarn("[runtimeSitemap] attachSitemapDownloadUI failed", e);
    return null;
  }
}
