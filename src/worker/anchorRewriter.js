...
export async function rewriteAnchors(
  article,
  contentBase,
  pagePath,
  opts = {},
  ...
  try {
      ...
      try {
        ...
        if (mdMatch) {
          ...
          try {
            ...
            debugWarn("[anchorRewriter] resolve mdPath failed", err);
          }
          continue;
        }
        try {
              ...
              try {
                if (mdToSlug?.has?.(rel)) slugKey = mdToSlug?.get?.(rel);
                  ...
                  try {
                    ...
                    debugWarn(
                      "[anchorRewriter] mdToSlug baseName check failed",
                      e,
                    );
                  }
                }
              } catch (err) {
                debugWarn("[anchorRewriter] mdToSlug access check failed", err);
              }
              if (!slugKey) {
                try {
                  const baseName = String(rel ?? "").replace(/^.*\//, "");
          ...
          debugWarn("[anchorRewriter] resolving href to URL failed", err);
        }
      } catch (err) {
        debugWarn("[anchorRewriter] processing anchor failed", err);
      }
    }
            ...
            if (basename) {
              ...
              if (candidate) {
                try {
                  ...
                  debugWarn(
                    "[anchorRewriter] fallback slug mapping failed",
                    err,
                  );
                }
              }
        ...
        await runWithConcurrency(
          ...
          async (rel) => {
            try {
                ...
                if (m2 && m2[1]) {
                  ...
                  if (candidate) {
                    try {
                      ...
                      debugWarn(
                        "[anchorRewriter] set slug mapping failed",
                        err,
                      );
                    }
                  }
              ...
              debugWarn(
                "[anchorRewriter] fetchMarkdown for md pending failed",
                err,
              );
            }
          },
            ...
            if (basename) {
              ...
              if (candidate) {
                try {
                  ...
                  debugWarn(
                    "[anchorRewriter] fallback html slug mapping failed",
                    err,
                  );
                }
              }
            ...
            try {
              ...
              if (res?.raw) {
                try {
                  ...
                  if (titleText) {
                    ...
                    if (slugKey) {
                      try {
                        ...
                        debugWarn(
                          "[anchorRewriter] set html slug mapping failed",
                          err,
                        );
                      }
                    }
                  ...
                  debugWarn("[anchorRewriter] parse fetched HTML failed", err);
                }
              }
            } catch (err) {
              debugWarn(
                "[anchorRewriter] fetchMarkdown for html pending failed",
                err,
              );
            }
          },
    ...
    for (const info of anchorInfo) {
      ...
      try {
        ...
        debugWarn("[anchorRewriter] mdToSlug access failed", err);
      }
      if (slug) {
        const urlVal = opts?.canonical
    ...
    for (const info of htmlAnchorInfo) {
      ...
      try {
        ...
        debugWarn(
          "[anchorRewriter] mdToSlug access failed for html anchor",
          err,
        );
      }
      if (!slug) {
        try {
          ...
          debugWarn(
            "[anchorRewriter] mdToSlug baseName access failed for html anchor",
            err,
          );
        }
      }
    ...
    debugWarn("[anchorRewriter] rewriteAnchors failed", err);
  }
}
