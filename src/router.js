...
function _fetchMd(path, base, signal) {
  try {
    if (
      typeof fetchMarkdown === "function" &&
      typeof fetchMarkdown.length === "number" &&
      fetchMarkdown.length >= 3
    ) {
...
function _enforceResolutionCacheMax() {
  try {
    resolutionCache.maxEntries = RESOLUTION_CACHE_MAX;
  } catch (_e) {}
...
export function _clearIndexCache() {
  indexSet.clear();
  try {
    setIndexPathsRefreshed(false);
  } catch (e) {
    debugWarn("[router] _clearIndexCache: refreshIndexPaths reset failed", e);
  }
}
...
export function _purgeExpiredEntries() {
  ...
  for (const k of keys) {
    ...
    if (
      _isLegacyResolutionRecord(record) &&
      record.ts + RESOLUTION_CACHE_TTL < now
    ) {
...
async function tryDiscoverFromIndex(decoded, contentBase, signal) {
  ...
  try {
    if (typeof document !== "undefined" && document.getElementsByClassName) {
      ...
      addFrom("nimbi-site-navbar");
      addFrom("navbar");
      addFrom("nimbi-nav");
    } else {
      anchorsForIndex = Array.from(
        document.querySelectorAll(
          ".nimbi-site-navbar a, .navbar a, .nimbi-nav a",
        ),
      );
    }
  } catch (err) {
  ...
  for (const linkEl of Array.from(anchorsForIndex || [])) {
    ...
    try {
      ...
      if (
        baseName &&
        slugify(baseName.replace(/\.(html?|md)$/i, "")) === decoded
      )
        return u.toString();
  ...
  for (const candidate of localCandidates) {
    try {
      ...
      debugWarn("[router] fetchMarkdown during index discovery failed", e);
    }
  }
  return null;
}
...
export function buildPageCandidates(resolved) {
  ...
  if (String(resolved).includes(".md") || String(resolved).includes(".html")) {
    ...
    try {
      ...
      debugWarn("[router] buildPageCandidates failed during slug handling", e);
    }
  }
...
export async function fetchPageData(raw, contentBase) {
  ...
  try {
    try {
      incrementCounter("fetchPageData");
    } catch (_) {}
  } catch (_) {}
  // Cancel any previous in-flight fetchPageData and create a fresh controller
  try {
    if (
      _lastFetchPageDataController &&
      typeof _lastFetchPageDataController.abort === "function"
    ) {
      try {
        _lastFetchPageDataController.abort();
      } catch (_) {}
    }
  } catch (_) {}
  _lastFetchPageDataController =
  ...
  const originalWasExplicitEarly =
    String(originalRaw ?? "").includes(".md") ||
    String(originalRaw ?? "").includes(".html");
  ...
  if (cached) {
    ...
    if (
      !String(resolved).includes(".md") &&
      !String(resolved).includes(".html")
    ) {
      ...
      if (slugToMd.has(decoded)) {
        ...
        let idx = await tryDiscoverFromIndex(
          decoded,
          contentBase,
          _fetchController ? _fetchController.signal : undefined,
        );
        if (idx) {
          resolved = idx;
        } else if (
          (isIndexPathsRefreshed() && indexSet && indexSet.size) ||
    ...
    const explicitResolved =
      ...
      (resolved &&
        (resolved.startsWith("http://") ||
          ...
          resolved.startsWith("/")));
    allowCandidateProbing =
      (typeof notFoundPage === "string" && notFoundPage) ||
      slugToMd.has(resolved) ||
      (indexSet && indexSet.size) ||
      isIndexPathsRefreshed() ||
  ...
  try {
    if (
      ...
      const abs = resolved.startsWith("/")
        ? new URL(resolved, location.origin).toString()
        : resolved;
      try {
        const res = await fetch(
          abs,
          _fetchController ? { signal: _fetchController.signal } : undefined,
        );
        if (res && res.ok) {
          ...
          const looksLikeHtml =
            (ct && ct.indexOf && ct.indexOf("text/html") !== -1) ||
            rawLower.indexOf("<!doctype") !== -1 ||
            rawLower.indexOf("<html") !== -1;
          if (looksLikeHtml) {
            ...
            if (!originalWasExplicitEarly) {
              try {
                let relPath = abs;
                try {
                  relPath = new URL(abs).pathname.replace(/^\//, "");
                } catch (_) {
                  relPath = String(abs ?? "").replace(/^\//, "");
                }
                const alt = relPath.replace(/\.html$/i, ".md");
                try {
                  const mdData = await _fetchMd(
                    alt,
                    contentBase,
                    _fetchController ? _fetchController.signal : undefined,
                  );
                ...
                if (typeof notFoundPage === "string" && notFoundPage) {
                  try {
                    ...
                    if (nf && nf.raw) {
                      try {
                        markNotFound(nf.meta || {}, notFoundPage);
                      } catch (_e) {}
                      return { data: nf, pagePath: notFoundPage, anchor };
                    }
                  } catch (_e) {
                    /* ignore notFoundPage probe */
                  }
                }

                try {
                  fetchError = new Error(
                    "site shell detected (absolute fetch)",
                  );
                } catch (_e) {}
            ...
            const looksLikeSiteShell =
              rawLower.indexOf('<div id="app"') !== -1 ||
        ...
        /* ignore absolute fetch failures */
      }
    }
  } catch (_e) {
    /* ignore */
  ...
  try {
    if (_routerShouldLog()) {
      try {
        debugLog("[router-debug] fetchPageData candidates", {
          originalRaw,
          resolved,
          pageCandidates,
        });
  ...
  if (
    ...
    (String(resolved).includes(".md") || String(resolved).includes(".html"))
  ) {
    pageCandidates.push(resolved);
  }
  ...
  if (
    ...
    !originalWasExplicit &&
    !slugToMd.has(resolved) &&
    !slugToMd.has(decodeURIComponent(String(resolved ?? ""))) &&
    !String(resolved ?? "").includes("/")
  ) {
    ...
    for (const candidate of pageCandidates) {
      ...
      try {
              ...
              if (titleSlug !== requestedSlug) {
                ...
                try {
                  if (/\.html$/i.test(norm)) {
                    ...
                    // Modernized: use Set for membership
                    const pageCandidatesSet = new Set(pageCandidates);
                    if (pageCandidatesSet.has(alt)) {
                      try {
                        const mdData = await _fetchMd(
                          alt,
                          contentBase,
          ...
          if (!originalWasExplicit && /\.html$/i.test(norm)) {
            ...
            if (pageCandidatesSet.has(alt)) {
              try {
                ...
                const looksLikeHtml =
                  (data && data.isHtml) ||
                  /^(?:<!doctype|<html|<title|<h1)/i.test(rawSnippet) ||
                  rawSnippet.indexOf('<div id="app"') !== -1 ||
                  rawSnippet.indexOf("nimbi-") !== -1 ||
                  rawSnippet.indexOf("nimbi") !== -1 ||
                  rawSnippet.indexOf("initcms(") !== -1;
                if (looksLikeHtml) {
                  let accepted = false;
                  try {
                    const mdData = await _fetchMd(
                  ...
                  if (!accepted) {
                    ...
                    pagePath = null;
                    fetchError = new Error(
                      "site shell detected (candidate HTML rejected)",
                    );
                    continue;
                  }
                }
        ...
        try {
          if (_routerShouldLog()) {
            try {
              debugLog("[router-debug] fetchPageData accepted candidate", {
                candidate: norm,
                pagePath,
                isHtml: data && data.isHtml,
                snippet:
                  data && data.raw ? String(data.raw).slice(0, 160) : null,
              });
            } catch (_e) {}
          }
        } catch (_e) {}
        ...
        try {
          if (_routerShouldLog())
            debugWarn("[router] candidate fetch failed", {
              candidate,
              contentBase,
              err: (e && e.message) || e,
            });
  ...
  if (!data) {
    const fetchErrMsg =
      (fetchError && (fetchError.message || String(fetchError))) || null;
    const isExpectedMissing =
      fetchErrMsg &&
      /failed to fetch md|site shell detected/i.test(fetchErrMsg);
    try {
      if (_routerShouldLog()) {
        try {
          debugLog("[router-debug] fetchPageData no data", {
            originalRaw,
            resolved,
            pageCandidates,
    ...
    if (isExpectedMissing) {
      try {
        if (_routerShouldLog()) {
          try {
            debugWarn("[router] fetchPageData: no page data (expected)", {
              originalRaw,
              resolved,
              pageCandidates,
              contentBase,
      ...
      try {
        if (_routerShouldLog()) {
          try {
            debugError("[router] fetchPageData: no page data for", {
              originalRaw,
              resolved,
              pageCandidates,
              contentBase,
    ...
    if (typeof notFoundPage === "string" && notFoundPage) {
      try {
        ...
        /* ignore notFoundPage probe errors */
      }
    }
    try {
      if (
        ...
        try {
          ...
          if (_routerShouldLog())
            debugWarn("[router] attempting absolute HTML fetch fallback", abs);
          const res = await fetch(
            abs,
            _fetchController ? { signal: _fetchController.signal } : undefined,
          );
          if (res && res.ok) {
            ...
            if (!looksLikeHtml && _routerShouldLog()) {
              try {
                debugWarn("[router] absolute fetch returned non-HTML", () => ({
                  abs,
                  contentType: ct,
                  snippet: rawLower.slice(0, 200),
                }));
              } catch (_e) {}
            }
            if (looksLikeHtml) {
              const rawLowerForDir = (raw || "").toLowerCase();
              const looksLikeDirListing =
                /<title>\s*index of\b/i.test(raw) ||
                /<h1>\s*index of\b/i.test(raw) ||
                rawLowerForDir.indexOf("parent directory") !== -1 ||
                /<title>\s*directory listing/i.test(raw) ||
                /<h1>\s*directory listing/i.test(raw);
              if (looksLikeDirListing) {
                try {
                  if (_routerShouldLog())
                    debugWarn(
                      "[router] absolute fetch returned directory listing; treating as not found",
                      { abs },
                    );
                } catch (_e) {}
              } else {
                try {
                    ...
                    if (parser) {
                      ...
                      const rewrite = (attr, el) => {
                        try {
                          ...
                          try {
                            ...
                            debugWarn(
                              "[router] rewrite attribute failed",
                              attr,
                              err,
                            );
                          }
                        } catch (err) {
                          debugWarn("[router] rewrite helper failed", err);
                        }
                      };
                            ...
                            const mapped = parts
                              .map((p) => {
                                ...
                                if (
                                  ...
                                  return p;
                                try {
                                  const r = new URL(urlPart, absUrl).toString();
                      ...
                      const modified =
                        doc.documentElement && doc.documentElement.outerHTML
                          ...
                          : raw;
                      try {
                        if (_routerShouldLog() && rewritten && rewritten.length)
                          debugWarn("[router] rewritten asset refs", {
                            abs,
                            rewritten,
                          });
                      } catch (_e) {}
                      return {
                        data: { raw: modified, isHtml: true },
                        pagePath: String(originalRaw ?? ""),
                        anchor,
                      };
                    }
                  } catch (e) {
                    ...
                    modified = modified.replace(
                      ...
                      (m, url) => {
                        ...
                        if (
                          ...
                          return m;
                        try {
                          const r = new URL(url, absUrl).toString();
                          return `src="${r}"`;
                        } catch (err) {
                          return m;
                        }
                  ...
                  if (!/<base\s+[^>]*>/i.test(rawWithBase)) {
                    if (/<head[^>]*>/i.test(rawWithBase)) {
                      rawWithBase = rawWithBase.replace(
                        /(<head[^>]*>)/i,
                  ...
                  return {
                    data: { raw: rawWithBase, isHtml: true },
                    pagePath: String(originalRaw ?? ""),
                    anchor,
                  };
                } catch (e) {
                  return {
                    data: { raw, isHtml: true },
                    pagePath: String(originalRaw ?? ""),
                    anchor,
                  };
                }
          ...
          if (_routerShouldLog())
            debugWarn("[router] absolute HTML fetch fallback failed", err);
        }
      }
    } catch (_) {
    ...
    try {
          ...
          for (const abs of candidates) {
            try {
              const res = await fetch(
                ...
                Object.assign(
                  { method: "GET" },
                  _fetchController ? { signal: _fetchController.signal } : {},
                ),
              );
              if (res && res.ok) {
                const raw = await res.text();
      ...
      if (_routerShouldLog()) debugWarn("[router] assets fallback failed", e);
    }
