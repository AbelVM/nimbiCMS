...
export function buildCosmeticUrl(page, anchor = null, baseSearch = undefined) {
  ...
  try {
    ...
    if (params) {
      const qs = (typeof params === 'string' && params.startsWith('?')) ? params.slice(1) : params
      try {
...
export function parseHrefToRoute(href) {
  try {
    const u = new URL(href, typeof location !== 'undefined' ? location.href : 'http://localhost/')
    ...
    if (pageParam) {
      ...
      return { type: 'canonical', page: decodeURIComponent(pageParam), anchor, params }
    }
    ...
    return { type: 'path', page: (u.pathname || '').replace(/^\//, '') || null, anchor: u.hash ? u.hash.replace(/^#/, '') : null, params: u.search ? u.search.replace(/^\?/, '') : '' }
  } catch (e) {
