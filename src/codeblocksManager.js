  ...
  try {
    ...
    if (st.failures >= CDN_CIRCUIT_FAILURES) {
      ...
      if (!st.warned) {
        try {
          debugWarn(
            "[codeblocksManager] CDN circuit opened for " +
              host +
              "; skipping CDN imports temporarily",
          );
        } catch (_) {}
        st.warned = true;
      }
...
function _noteCdnSuccess(host) {
  try {
    if (host) _cdnCircuitState.delete(host);
  } catch (_) {}
}
 ...
 * @returns {void}
 */
export function clearLanguageImportCache() {
  clearImportCache();
  warnedFailedLanguageImports.clear();
}
 ...
 * @returns {void}
 */
export function setLanguageImportNegativeCacheTTL(ms) {
  ...
  loadSupportedLanguagesPromise = (async () => {
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (/\|\s*Language\s*\|/i.test(lines[i])) {
          headerIdx = i;
          break;
        }
      }
      ...
      let aliasesIdx = headerCols.findIndex((h) =>
        /alias|aliases|equivalent|alt|alternates?/i.test(h),
      );
      if (aliasesIdx === -1) aliasesIdx = 1;
      let canonicalIdx = headerCols.findIndex((h) =>
        /file|filename|module|module name|module-name|short|slug/i.test(h),
      );
      if (canonicalIdx === -1) {
      ...
      for (let i = headerIdx + 1; i < lines.length; i++) {
        ...
        const canonicalRaw = (cols[canonicalIdx] || cols[0] || "")
          .toString()
          .trim();
      ...
      try {
        ...
        debugWarn("[codeblocksManager] cleanup aliases failed", err);
      }
      try {
        let removed = 0;
        ...
        try {
          const sepKey = ":---------------------";
          if (SUPPORTED_HLJS_MAP.has(sepKey)) {
            SUPPORTED_HLJS_MAP.delete(sepKey);
            removed++;
          }
        } catch (err) {
          debugWarn("[codeblocksManager] remove sep key failed", err);
        }
        try {
          const keys = Array.from(SUPPORTED_HLJS_MAP.keys()).sort();
        } catch (err) {
          debugWarn("[codeblocksManager] compute supported keys failed", err);
        }
      } catch (_) {
        debugWarn("[codeblocksManager] ignored error", _);
      }
    } catch (err) {
      debugWarn("[codeblocksManager] loadSupportedLanguages failed", err);
    }
  })();
...
const warnedFailedLanguageImports = new Set();

function warnLanguageImportFailureOnce(name, candidates, err) {
  const key = String(name || "").toLowerCase();
  if (!key || warnedFailedLanguageImports.has(key)) return;
  warnedFailedLanguageImports.add(key);
  try {
    debugWarn(
      "[codeblocksManager] language import failed; using plaintext/highlightElement fallback",
      {
        language: name,
...
export async function registerLanguage(name, modulePath) {
  if (!loadSupportedLanguagesPromise) {
    (async () => {
      try {
        ...
        debugWarn(
          "[codeblocksManager] loadSupportedLanguages (IIFE) failed",
          err,
        );
      }
    })();
  ...
  if (loadSupportedLanguagesPromise) {
    try {
      ...
      /* ignore load failures; fallback behavior below */
    }
  }
  ...
  try {
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (mod) {
          ...
          try {
            const registerName =
              (SUPPORTED_HLJS_MAP.size && SUPPORTED_HLJS_MAP.get(name)) ||
              candidate ||
              name;
    ...
    if (lastErr) {
      warnLanguageImportFailureOnce(name, candidates, lastErr);
      return false;
    }
    if (candidates.length) {
      warnLanguageImportFailureOnce(name, candidates, null);
    }
    return false;
  } catch (_) {
    warnLanguageImportFailureOnce(name, [], _);
    return false;
  }
}
...
export function observeCodeBlocks(root) {
  ...
  if (!loadSupportedLanguagesPromise) {
    (async () => {
      try {
        ...
        debugWarn(
          "[codeblocksManager] loadSupportedLanguages (observer) failed",
          _,
        );
      }
    })();
      ...
      (entries, obs) => {
        entries.forEach((entry) => {
          ...
          try {
            ...
            debugWarn("[codeblocksManager] observer unobserve failed", err);
          }
          (async () => {
            try {
              const cls =
              ...
              if (match && match[1]) {
                ...
                const canonical =
                  (SUPPORTED_HLJS_MAP.size &&
                    (SUPPORTED_HLJS_MAP.get(mapped) ||
                      SUPPORTED_HLJS_MAP.get(String(mapped).toLowerCase()))) ||
                  mapped;
                try {
                  await registerLanguage(canonical);
                } catch (err) {
                  debugWarn("[codeblocksManager] registerLanguage failed", err);
                }
                try {
                  try {
                  ...
                  try {
                    if (el?.dataset?.highlighted) delete el.dataset.highlighted;
                  } catch (_) {}
                  hljs.highlightElement(el);
                } catch (err) {
                  debugWarn(
                    "[codeblocksManager] hljs.highlightElement failed",
                    err,
                  );
                }
              } else {
                try {
                  ...
                  try {
                    ...
                    try {
                      ...
                      debugWarn(
                        "[codeblocksManager] fallback highlightElement failed",
                        e,
                      );
                    }
                  }
                  ...
                  debugWarn(
                    "[codeblocksManager] auto-detect plaintext failed",
                    err,
                  );
                }
              }
              ...
              debugWarn(
                "[codeblocksManager] observer entry processing failed",
                err,
              );
            }
          })();
      ...
      try {
        ...
        if (match && match[1]) {
          ...
          try {
            ...
            debugWarn(
              "[codeblocksManager] registerLanguage failed (no observer)",
              err,
            );
          }
        }
        try {
          ...
          debugWarn(
            "[codeblocksManager] hljs.highlightElement failed (no observer)",
            err,
          );
        }
      } catch (_) {
        debugWarn(
          "[codeblocksManager] loadSupportedLanguages fallback ignored error",
          _,
        );
      }
    });
  ...
  blocks.forEach((b) => {
    try {
      ...
      debugWarn("[codeblocksManager] observe failed", err);
    }
  });
}
/**
...
export function setHighlightTheme(theme, { useCdn = true } = {}) {
  const existing =
    ...
    document.head.querySelector
      ? document.head.querySelector("link[data-hl-theme]")
      : typeof document !== "undefined"
        ? document.querySelector("link[data-hl-theme]")
        : null;
  const existingTheme = existing?.getAttribute
    ? existing.getAttribute("data-hl-theme")
    : null;

  const requested =
    theme === undefined || theme === null ? "default" : String(theme);
