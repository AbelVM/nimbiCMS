  ...
  if (/^[a-z][a-z0-9+.-]*:/i.test(href)) {
    if (contentBase && typeof contentBase === 'string') {
      try {
        ...
        if (target.origin === baseUrl.origin) return !target.pathname.startsWith(baseUrl.pathname)
      } catch (_) {}
    }
    return true
  }
...
async function _runWithConcurrency(items, worker, concurrency = DEFAULT_CONCURRENCY) {
...
async function _crawlAllMarkdown(contentBase, maxQueue = DEFAULT_MAX_CRAWL_QUEUE) {
  ...
  while (queue.length && queue.length <= maxQueue) {
    ...
    while ((match = hrefRegex.exec(html))) {
      const href = match[1]
      if (!href || _isExternalHref(href, baseAbs) || href.startsWith('..') || href.includes('/../')) continue
...
export async function buildSearchIndex(contentBase, indexDepth = 1, noIndexing = undefined, seedPaths = undefined) {
  ...
  _indexPromise = (async () => {
    const excludes = Array.isArray(noIndexing) ? new Set(noIndexing.map((p) => _sanitizePath(p))) : new Set()
    const seeds = Array.isArray(seedPaths) ? seedPaths.map((p) => _sanitizePath(p)).filter(Boolean) : []
    ...
    const allPaths = Array.from(new Set(discovered.concat(seeds)))
      ...
      .filter((p) => !Array.from(excludes).some((x) => x && (p === x || p.startsWith(x + '/'))))
...
async function getSlugRuntime() {
  if (!_runtimePromise) _runtimePromise = import('./slugManager.js')
