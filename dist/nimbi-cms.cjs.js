...
`,1)[0],U;if(p=I,this.options.pedantic?(p=p.replace(this.rules.other.listReplaceNesting,"  "),U=p):U=p.replace(this.rules.other.tabCharGlobal,"    "),b.test(p)||y.test(p)||A.test(p)||C.test(p)||g.test(p)||m.test(p))break;if(U.search(this.rules.other.nonSpaceChar)>=h||!p.trim())o+=`
