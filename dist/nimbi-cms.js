(function(Be,Ht){typeof exports=="object"&&typeof module<"u"?Ht(exports):typeof define=="function"&&define.amd?define(["exports"],Ht):(Be=typeof globalThis<"u"?globalThis:Be||self,Ht(Be.nimbiCMS={}))})(this,(function(Be){"use strict";let Ht=0;const gi=Object.create(null);function mi(e){try{const t=Number(e);Ht=Number.isFinite(t)&&t>=0?Math.max(0,Math.min(3,Math.floor(t))):0}catch{Ht=0}}function Ft(e=1){try{return Number(Ht)>=Number(e||1)}catch{return!1}}function pr(){return Ft(1)}function Nn(...e){try{if(!Ft(1)||!console||typeof console.error!="function")return;const t=e.map(n=>typeof n=="function"?n():n);console.error(...t)}catch{}}function _(...e){try{if(!Ft(2)||!console||typeof console.warn!="function")return;const t=e.map(n=>typeof n=="function"?n():n);console.warn(...t)}catch{}}function Lt(...e){try{if(!Ft(3)||!console||typeof console.info!="function")return;const t=e.map(n=>typeof n=="function"?n():n);console.info(...t)}catch{}}function Tt(...e){try{if(!Ft(3)||!console||typeof console.log!="function")return;const t=e.map(n=>typeof n=="function"?n():n);console.log(...t)}catch{}}function yi(e){try{if(!pr())return;const t=String(e||"");if(!t)return;gi[t]=(gi[t]||0)+1}catch{}}function bi(e){try{if(typeof globalThis>"u"||!globalThis.__nimbiCMSDebug)return;const t=String(e||"");if(!t)return;try{globalThis.__nimbiCMSDebug[t]=(globalThis.__nimbiCMSDebug[t]||0)+1}catch{}}catch{}}const cn={onPageLoad:[],onNavBuild:[],transformHtml:[]};function Bn(e,t){if(!Object.prototype.hasOwnProperty.call(cn,e))throw new Error('Unknown hook "'+e+'"');if(typeof t!="function")throw new TypeError("hook callback must be a function");cn[e].push(t)}function Xa(e){Bn("onPageLoad",e)}function Qa(e){Bn("onNavBuild",e)}function Ka(e){Bn("transformHtml",e)}async function gr(e,t){const n=cn[e]||[];for(const i of n)try{await i(t)}catch(r){try{_("[nimbi-cms] runHooks callback failed",r)}catch{}}}function Va(){Object.keys(cn).forEach(e=>{cn[e].length=0})}function wi(e){return e&&e.__esModule&&Object.prototype.hasOwnProperty.call(e,"default")?e.default:e}var mr,_i;function Ya(){if(_i)return mr;_i=1;function e(x){return x instanceof Map?x.clear=x.delete=x.set=function(){throw new Error("map is read-only")}:x instanceof Set&&(x.add=x.clear=x.delete=function(){throw new Error("set is read-only")}),Object.freeze(x),Object.getOwnPropertyNames(x).forEach(N=>{const ee=x[N],we=typeof ee;(we==="object"||we==="function")&&!Object.isFrozen(ee)&&e(ee)}),x}class t{constructor(N){N.data===void 0&&(N.data={}),this.data=N.data,this.isMatchIgnored=!1}ignoreMatch(){this.isMatchIgnored=!0}}function n(x){return x.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#x27;")}function i(x,...N){const ee=Object.create(null);for(const we in x)ee[we]=x[we];return N.forEach(function(we){for(const We in we)ee[We]=we[We]}),ee}const r="</span>",a=x=>!!x.scope,s=(x,{prefix:N})=>{if(x.startsWith("language:"))return x.replace("language:","language-");if(x.includes(".")){const ee=x.split(".");return[`${N}${ee.shift()}`,...ee.map((we,We)=>`${we}${"_".repeat(We+1)}`)].join(" ")}return`${N}${x}`};class l{constructor(N,ee){this.buffer="",this.classPrefix=ee.classPrefix,N.walk(this)}addText(N){this.buffer+=n(N)}openNode(N){if(!a(N))return;const ee=s(N.scope,{prefix:this.classPrefix});this.span(ee)}closeNode(N){a(N)&&(this.buffer+=r)}value(){return this.buffer}span(N){this.buffer+=`<span class="${N}">`}}const o=(x={})=>{const N={children:[]};return Object.assign(N,x),N};class c{constructor(){this.rootNode=o(),this.stack=[this.rootNode]}get top(){return this.stack[this.stack.length-1]}get root(){return this.rootNode}add(N){this.top.children.push(N)}openNode(N){const ee=o({scope:N});this.add(ee),this.stack.push(ee)}closeNode(){if(this.stack.length>1)return this.stack.pop()}closeAllNodes(){for(;this.closeNode(););}toJSON(){return JSON.stringify(this.rootNode,null,4)}walk(N){return this.constructor._walk(N,this.rootNode)}static _walk(N,ee){return typeof ee=="string"?N.addText(ee):ee.children&&(N.openNode(ee),ee.children.forEach(we=>this._walk(N,we)),N.closeNode(ee)),N}static _collapse(N){typeof N!="string"&&N.children&&(N.children.every(ee=>typeof ee=="string")?N.children=[N.children.join("")]:N.children.forEach(ee=>{c._collapse(ee)}))}}class u extends c{constructor(N){super(),this.options=N}addText(N){N!==""&&this.add(N)}startScope(N){this.openNode(N)}endScope(){this.closeNode()}__addSublanguage(N,ee){const we=N.root;ee&&(we.scope=`language:${ee}`),this.add(we)}toHTML(){return new l(this,this.options).value()}finalize(){return this.closeAllNodes(),!0}}function h(x){return x?typeof x=="string"?x:x.source:null}function d(x){return y("(?=",x,")")}function p(x){return y("(?:",x,")*")}function f(x){return y("(?:",x,")?")}function y(...x){return x.map(ee=>h(ee)).join("")}function g(x){const N=x[x.length-1];return typeof N=="object"&&N.constructor===Object?(x.splice(x.length-1,1),N):{}}function m(...x){return"("+(g(x).capture?"":"?:")+x.map(we=>h(we)).join("|")+")"}function b(x){return new RegExp(x.toString()+"|").exec("").length-1}function w(x,N){const ee=x&&x.exec(N);return ee&&ee.index===0}const k=/\[(?:[^\\\]]|\\.)*\]|\(\??|\\([1-9][0-9]*)|\\./;function S(x,{joinWith:N}){let ee=0;return x.map(we=>{ee+=1;const We=ee;let Ze=h(we),se="";for(;Ze.length>0;){const ae=k.exec(Ze);if(!ae){se+=Ze;break}se+=Ze.substring(0,ae.index),Ze=Ze.substring(ae.index+ae[0].length),ae[0][0]==="\\"&&ae[1]?se+="\\"+String(Number(ae[1])+We):(se+=ae[0],ae[0]==="("&&ee++)}return se}).map(we=>`(${we})`).join(N)}const v=/\b\B/,E="[a-zA-Z]\\w*",B="[a-zA-Z_]\\w*",H="\\b\\d+(\\.\\d+)?",Z="(-?)(\\b0[xX][a-fA-F0-9]+|(\\b\\d+(\\.\\d*)?|\\.\\d+)([eE][-+]?\\d+)?)",$="\\b(0b[01]+)",j="!|!=|!==|%|%=|&|&&|&=|\\*|\\*=|\\+|\\+=|,|-|-=|/=|/|:|;|<<|<<=|<=|<|===|==|=|>>>=|>>=|>=|>>>|>>|>|\\?|\\[|\\{|\\(|\\^|\\^=|\\||\\|=|\\|\\||~",G=(x={})=>{const N=/^#![ ]*\//;return x.binary&&(x.begin=y(N,/.*\b/,x.binary,/\b.*/)),i({scope:"meta",begin:N,end:/$/,relevance:0,"on:begin":(ee,we)=>{ee.index!==0&&we.ignoreMatch()}},x)},ie={begin:"\\\\[\\s\\S]",relevance:0},F={scope:"string",begin:"'",end:"'",illegal:"\\n",contains:[ie]},L={scope:"string",begin:'"',end:'"',illegal:"\\n",contains:[ie]},D={begin:/\b(a|an|the|are|I'm|isn't|don't|doesn't|won't|but|just|should|pretty|simply|enough|gonna|going|wtf|so|such|will|you|your|they|like|more)\b/},U=function(x,N,ee={}){const we=i({scope:"comment",begin:x,end:N,contains:[]},ee);we.contains.push({scope:"doctag",begin:"[ ]*(?=(TODO|FIXME|NOTE|BUG|OPTIMIZE|HACK|XXX):)",end:/(TODO|FIXME|NOTE|BUG|OPTIMIZE|HACK|XXX):/,excludeBegin:!0,relevance:0});const We=m("I","a","is","so","us","to","at","if","in","it","on",/[A-Za-z]+['](d|ve|re|ll|t|s|n)/,/[A-Za-z]+[-][a-z]+/,/[A-Za-z][a-z]{2,}/);return we.contains.push({begin:y(/[ ]+/,"(",We,/[.]?[:]?([.][ ]|[ ])/,"){3}")}),we},le=U("//","$"),te=U("/\\*","\\*/"),ce=U("#","$"),_e={scope:"number",begin:H,relevance:0},Ee={scope:"number",begin:Z,relevance:0},Fe={scope:"number",begin:$,relevance:0},xe={scope:"regexp",begin:/\/(?=[^/\n]*\/)/,end:/\/[gimuy]*/,contains:[ie,{begin:/\[/,end:/\]/,relevance:0,contains:[ie]}]},Ue={scope:"title",begin:E,relevance:0},R={scope:"title",begin:B,relevance:0},z={begin:"\\.\\s*"+B,relevance:0};var C=Object.freeze({__proto__:null,APOS_STRING_MODE:F,BACKSLASH_ESCAPE:ie,BINARY_NUMBER_MODE:Fe,BINARY_NUMBER_RE:$,COMMENT:U,C_BLOCK_COMMENT_MODE:te,C_LINE_COMMENT_MODE:le,C_NUMBER_MODE:Ee,C_NUMBER_RE:Z,END_SAME_AS_BEGIN:function(x){return Object.assign(x,{"on:begin":(N,ee)=>{ee.data._beginMatch=N[1]},"on:end":(N,ee)=>{ee.data._beginMatch!==N[1]&&ee.ignoreMatch()}})},HASH_COMMENT_MODE:ce,IDENT_RE:E,MATCH_NOTHING_RE:v,METHOD_GUARD:z,NUMBER_MODE:_e,NUMBER_RE:H,PHRASAL_WORDS_MODE:D,QUOTE_STRING_MODE:L,REGEXP_MODE:xe,RE_STARTERS_RE:j,SHEBANG:G,TITLE_MODE:Ue,UNDERSCORE_IDENT_RE:B,UNDERSCORE_TITLE_MODE:R});function T(x,N){x.input[x.index-1]==="."&&N.ignoreMatch()}function I(x,N){x.className!==void 0&&(x.scope=x.className,delete x.className)}function K(x,N){N&&x.beginKeywords&&(x.begin="\\b("+x.beginKeywords.split(" ").join("|")+")(?!\\.)(?=\\b|\\s)",x.__beforeBegin=T,x.keywords=x.keywords||x.beginKeywords,delete x.beginKeywords,x.relevance===void 0&&(x.relevance=0))}function P(x,N){Array.isArray(x.illegal)&&(x.illegal=m(...x.illegal))}function q(x,N){if(x.match){if(x.begin||x.end)throw new Error("begin & end are not supported with match");x.begin=x.match,delete x.match}}function M(x,N){x.relevance===void 0&&(x.relevance=1)}const O=(x,N)=>{if(!x.beforeMatch)return;if(x.starts)throw new Error("beforeMatch cannot be used with starts");const ee=Object.assign({},x);Object.keys(x).forEach(we=>{delete x[we]}),x.keywords=ee.keywords,x.begin=y(ee.beforeMatch,d(ee.begin)),x.starts={relevance:0,contains:[Object.assign(ee,{endsParent:!0})]},x.relevance=0,delete ee.beforeMatch},J=["of","and","for","in","not","or","if","then","parent","list","value"],X="keyword";function me(x,N,ee=X){const we=Object.create(null);return typeof x=="string"?We(ee,x.split(" ")):Array.isArray(x)?We(ee,x):Object.keys(x).forEach(function(Ze){Object.assign(we,me(x[Ze],N,Ze))}),we;function We(Ze,se){N&&(se=se.map(ae=>ae.toLowerCase())),se.forEach(function(ae){const be=ae.split("|");we[be[0]]=[Ze,Me(be[0],be[1])]})}}function Me(x,N){return N?Number(N):ye(x)?0:1}function ye(x){return J.includes(x.toLowerCase())}const ge={},Le=x=>{console.error(x)},Ve=(x,...N)=>{console.log(`WARN: ${x}`,...N)},qe=(x,N)=>{ge[`${x}/${N}`]||(console.log(`Deprecated as of ${x}. ${N}`),ge[`${x}/${N}`]=!0)},St=new Error;function $n(x,N,{key:ee}){let we=0;const We=x[ee],Ze={},se={};for(let ae=1;ae<=N.length;ae++)se[ae+we]=We[ae],Ze[ae+we]=!0,we+=b(N[ae-1]);x[ee]=se,x[ee]._emit=Ze,x[ee]._multi=!0}function Ct(x){if(Array.isArray(x.begin)){if(x.skip||x.excludeBegin||x.returnBegin)throw Le("skip, excludeBegin, returnBegin not compatible with beginScope: {}"),St;if(typeof x.beginScope!="object"||x.beginScope===null)throw Le("beginScope must be object"),St;$n(x,x.begin,{key:"beginScope"}),x.begin=S(x.begin,{joinWith:""})}}function Nl(x){if(Array.isArray(x.end)){if(x.skip||x.excludeEnd||x.returnEnd)throw Le("skip, excludeEnd, returnEnd not compatible with endScope: {}"),St;if(typeof x.endScope!="object"||x.endScope===null)throw Le("endScope must be object"),St;$n(x,x.end,{key:"endScope"}),x.end=S(x.end,{joinWith:""})}}function Bl(x){x.scope&&typeof x.scope=="object"&&x.scope!==null&&(x.beginScope=x.scope,delete x.scope)}function Ol(x){Bl(x),typeof x.beginScope=="string"&&(x.beginScope={_wrap:x.beginScope}),typeof x.endScope=="string"&&(x.endScope={_wrap:x.endScope}),Ct(x),Nl(x)}function jl(x){function N(se,ae){return new RegExp(h(se),"m"+(x.case_insensitive?"i":"")+(x.unicodeRegex?"u":"")+(ae?"g":""))}class ee{constructor(){this.matchIndexes={},this.regexes=[],this.matchAt=1,this.position=0}addRule(ae,be){be.position=this.position++,this.matchIndexes[this.matchAt]=be,this.regexes.push([be,ae]),this.matchAt+=b(ae)+1}compile(){this.regexes.length===0&&(this.exec=()=>null);const ae=this.regexes.map(be=>be[1]);this.matcherRe=N(S(ae,{joinWith:"|"}),!0),this.lastIndex=0}exec(ae){this.matcherRe.lastIndex=this.lastIndex;const be=this.matcherRe.exec(ae);if(!be)return null;const Ye=be.findIndex((In,oi)=>oi>0&&In!==void 0),Ge=this.matchIndexes[Ye];return be.splice(0,Ye),Object.assign(be,Ge)}}class we{constructor(){this.rules=[],this.multiRegexes=[],this.count=0,this.lastIndex=0,this.regexIndex=0}getMatcher(ae){if(this.multiRegexes[ae])return this.multiRegexes[ae];const be=new ee;return this.rules.slice(ae).forEach(([Ye,Ge])=>be.addRule(Ye,Ge)),be.compile(),this.multiRegexes[ae]=be,be}resumingScanAtSamePosition(){return this.regexIndex!==0}considerAll(){this.regexIndex=0}addRule(ae,be){this.rules.push([ae,be]),be.type==="begin"&&this.count++}exec(ae){const be=this.getMatcher(this.regexIndex);be.lastIndex=this.lastIndex;let Ye=be.exec(ae);if(this.resumingScanAtSamePosition()&&!(Ye&&Ye.index===this.lastIndex)){const Ge=this.getMatcher(0);Ge.lastIndex=this.lastIndex+1,Ye=Ge.exec(ae)}return Ye&&(this.regexIndex+=Ye.position+1,this.regexIndex===this.count&&this.considerAll()),Ye}}function We(se){const ae=new we;return se.contains.forEach(be=>ae.addRule(be.begin,{rule:be,type:"begin"})),se.terminatorEnd&&ae.addRule(se.terminatorEnd,{type:"end"}),se.illegal&&ae.addRule(se.illegal,{type:"illegal"}),ae}function Ze(se,ae){const be=se;if(se.isCompiled)return be;[I,q,Ol,O].forEach(Ge=>Ge(se,ae)),x.compilerExtensions.forEach(Ge=>Ge(se,ae)),se.__beforeBegin=null,[K,P,M].forEach(Ge=>Ge(se,ae)),se.isCompiled=!0;let Ye=null;return typeof se.keywords=="object"&&se.keywords.$pattern&&(se.keywords=Object.assign({},se.keywords),Ye=se.keywords.$pattern,delete se.keywords.$pattern),Ye=Ye||/\w+/,se.keywords&&(se.keywords=me(se.keywords,x.case_insensitive)),be.keywordPatternRe=N(Ye,!0),ae&&(se.begin||(se.begin=/\B|\b/),be.beginRe=N(be.begin),!se.end&&!se.endsWithParent&&(se.end=/\B|\b/),se.end&&(be.endRe=N(be.end)),be.terminatorEnd=h(be.end)||"",se.endsWithParent&&ae.terminatorEnd&&(be.terminatorEnd+=(se.end?"|":"")+ae.terminatorEnd)),se.illegal&&(be.illegalRe=N(se.illegal)),se.contains||(se.contains=[]),se.contains=[].concat(...se.contains.map(function(Ge){return ql(Ge==="self"?se:Ge)})),se.contains.forEach(function(Ge){Ze(Ge,be)}),se.starts&&Ze(se.starts,ae),be.matcher=We(be),be}if(x.compilerExtensions||(x.compilerExtensions=[]),x.contains&&x.contains.includes("self"))throw new Error("ERR: contains `self` is not supported at the top-level of a language.  See documentation.");return x.classNameAliases=i(x.classNameAliases||{}),Ze(x)}function Na(x){return x?x.endsWithParent||Na(x.starts):!1}function ql(x){return x.variants&&!x.cachedVariants&&(x.cachedVariants=x.variants.map(function(N){return i(x,{variants:null},N)})),x.cachedVariants?x.cachedVariants:Na(x)?i(x,{starts:x.starts?i(x.starts):null}):Object.isFrozen(x)?i(x):x}var Hl="11.11.1";class Fl extends Error{constructor(N,ee){super(N),this.name="HTMLInjectionError",this.html=ee}}const si=n,Ba=i,Oa=Symbol("nomatch"),Dl=7,ja=function(x){const N=Object.create(null),ee=Object.create(null),we=[];let We=!0;const Ze="Could not find the language '{}', did you forget to load/include a language module?",se={disableAutodetect:!0,name:"Plain text",contains:[]};let ae={ignoreUnescapedHTML:!1,throwUnescapedHTML:!1,noHighlightRe:/^(no-?highlight)$/i,languageDetectRe:/\blang(?:uage)?-([\w-]+)\b/i,classPrefix:"hljs-",cssSelector:"pre code",languages:null,__emitter:u};function be(Q){return ae.noHighlightRe.test(Q)}function Ye(Q){let he=Q.className+" ";he+=Q.parentNode?Q.parentNode.className:"";const Te=ae.languageDetectRe.exec(he);if(Te){const Oe=jt(Te[1]);return Oe||(Ve(Ze.replace("{}",Te[1])),Ve("Falling back to no-highlight mode for this block.",Q)),Oe?Te[1]:"no-highlight"}return he.split(/\s+/).find(Oe=>be(Oe)||jt(Oe))}function Ge(Q,he,Te){let Oe="",Qe="";typeof he=="object"?(Oe=Q,Te=he.ignoreIllegals,Qe=he.language):(qe("10.7.0","highlight(lang, code, ...args) has been deprecated."),qe("10.7.0",`Please use highlight(code, options) instead.
