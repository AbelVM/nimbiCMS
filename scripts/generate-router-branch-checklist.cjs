...
const routerKey = keys.find(k => k.endsWith('/src/router.js') || k.endsWith('src/router.js') || k.endsWith('\\src\\router.js'));
if (!routerKey) {
  console.error('Could not find coverage entry for src/router.js among keys:', keys.slice(0,20));
  process.exit(4);
}

const entry = cov[routerKey];
...
const out = { routerKey, count: rows.length, rows };
const outPath = path.resolve(__dirname, '..', 'coverage', 'router-branch-checklist.json');
fs.writeFileSync(outPath, JSON.stringify(out, null, 2), 'utf8');
console.log('WROTE', outPath);
console.log(JSON.stringify({routerKey, top: rows.slice(0,30)}, null, 2));
