...
const key = Object.keys(c).find(k => k.endsWith('/src/nav.js'))
if (!key) { console.error('nav.js not found in coverage'); process.exit(1) }
const d = c[key]
...
if (total === 0) { console.log('no branch data for nav.js') } else {
  console.log('nav branch coverage:', (covered/total*100).toFixed(2)+'%','(',covered,'/',total,')')
}
