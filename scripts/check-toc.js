#!/usr/bin/env node
/**
 * check-toc.js — verify README table-of-contents anchors match headings.
 *
 * Usage:
 *   node scripts/check-toc.js [README.md]
 */
const fs = require('fs');
...
function slugify(text) {
  ...
  // Remove punctuation characters except spaces, hyphens, and underscores
  s = s.replace(/[\u2000-\u206F\u2E00-\u2E7F\'"\!\$%\(\)\*\+,\./:;<>\=?@\[\\\]^`{|}~&—–]/g, '');
  s = s.replace(/\s+/g, '-');
  s = s.replace(/-+/g, '-');
  return s;
