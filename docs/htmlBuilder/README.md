[**nimbi-cms**](../README.md)

***

[nimbi-cms](../README.md) / htmlBuilder

# htmlBuilder

## Type Aliases

- [ArticleResult](type-aliases/ArticleResult.md)
- [NavItem](type-aliases/NavItem.md)
- [ParsedPage](type-aliases/ParsedPage.md)

## Functions

- [\_computeSlug](functions/computeSlug.md)
- [\_ensureLanguages](functions/ensureLanguages.md)
- [\_parseHtml](functions/parseHtml.md)
- [\_parseMarkdown](functions/parseMarkdown.md)
- [attachTocClickHandler](functions/attachTocClickHandler.md)
- [buildTocElement](functions/buildTocElement.md)
- [createNavTree](functions/createNavTree.md)
- [ensureScrollTopButton](functions/ensureScrollTopButton.md)
- [executeEmbeddedScripts](functions/executeEmbeddedScripts.md)
- [initAnchorWorker](functions/initAnchorWorker.md)
- [preMapMdSlugs](functions/preMapMdSlugs.md)
- [prepareArticle](functions/prepareArticle.md)
- [preScanHtmlSlugs](functions/preScanHtmlSlugs.md)
- [renderNotFound](functions/renderNotFound.md)
- [rewriteAnchors](functions/rewriteAnchors.md)
- [rewriteAnchorsWorker](functions/rewriteAnchorsWorker.md)
- [scrollToAnchorOrTop](functions/scrollToAnchorOrTop.md)

## References

### \_rewriteAnchors

Renames and re-exports [rewriteAnchors](functions/rewriteAnchors.md)

***

### \_rewriteAnchorsWorker

Renames and re-exports [rewriteAnchorsWorker](functions/rewriteAnchorsWorker.md)
